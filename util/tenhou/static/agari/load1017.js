...
function sprintSyanten(n){
	return n==-1?"화료":n==0?"텐파이":n+"샹텐";
}
function sprintSyanten2(a,b){
	if (b && a[0]!=a[1]) return "표준형"+sprintSyanten(a[0])+" / 일반형"+sprintSyanten(a[1]);
...
function printTehai(qt,q){
	...
	case "q":s+="표준형(치또이/국사 포함) 계산 결과"    +" / <a href=\""+"javascript:main(document.getElementById('tehaiInput').value, 'p');"+"\">표준형</a><br>";break;
	case "p":s+="일반형(치또이/국사 제외) 계산 결과"+" / <a href=\""+"javascript:main(document.getElementById('tehaiInput').value, 'q');"+"\">일반형</a><br>";break;
	}
	...
	tehai+="("+Math.floor(q.length/2)+"?)";
	if (syanten_org01[0]==-1) tehai+=" / <a href=\"javascript:main('', _qt);\" >새 손패 생성</a>";
	tehai+="<br/>";
...
//		s+="<br>time="+tt+" eval="+SYANTEN.n_eval;
	...
	if (v[34] && v[34].n){
		...
		trtd.push(v[34]);
		tehai+="<br><br><a href=\"?"+qt+"="+v[34].q+"\">무작위 쓰모</a>";
	}

	trtd.sort(function(a,b){return b.n-a.n;});
	s+="<table cellpadding=2 cellspacing=0 >";
	var tdisp=(syanten_org<=0?"대기":"쓰모");
	for(i=0;i<trtd.length;++i){
		var da34=trtd[i].da;
		s+="<tr id=mda"+da34+" ><td>";
		if (da34<34) s+="?</td><td>"+img2(MPSZ.fromHai136(da34*4+1))+"</td><td>";
		s+=tdisp+"[</td><td>";
