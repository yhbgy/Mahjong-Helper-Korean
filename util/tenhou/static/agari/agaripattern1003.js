...
function AGARIPATTERN(){
	...
	this.v=[{atama34:-1,mmmm35:0},{atama34:-1,mmmm35:0},{atama34:-1,mmmm35:0},{atama34:-1,mmmm35:0}]; // 설명
	// 설명
}
...
//	isChiitoi:function(){return this.v[3].mmmm35==0xFFFFFFFF;},
	...
	getAgariPattern:function(c,n){
		...
		if (k0==1) return ok; // 설명
		var k1=(n10+n11+n12)%3;
		if (k1==1) return ok; // 설명
		var k2=(n20+n21+n22)%3;
		if (k2==1) return ok; // 설명
		if ((k0==2)+(k1==2)+(k2==2)+
			(c[27]==2)+(c[28]==2)+(c[29]==2)+(c[30]==2)+
			(c[31]==2)+(c[32]==2)+(c[33]==2)!=1) return ok; // 설명
		if (j&8){ // 설명
...
//		document.write("n="+n0+" "+n1+" "+n2+" k="+k0+" "+k1+" "+k2+" kk="+kk0+" "+kk1+" "+kk2+" mmmm="+v[0].mmmm35+"<br>");
		if (j&4){ // 설명
			...
			}else if (e.GetMentsu(0,m0) && e.GetMentsu(1,m1) && e.GetMentsu(2,m2)) return true; // 설명
		}else if (k0==2){ // 설명
			...
			}else if (e.GetMentsu(1,m1) && e.GetMentsu(2,m2) && e.GetAtamaMentsu(kk0,0,m0)) return true; // 설명
		}else if (k1==2){ // 설명
			...
			}else if (e.GetMentsu(2,m2) && e.GetMentsu(0,m0) && e.GetAtamaMentsu(kk1,1,m1)) return true; // 설명
		}else if (k2==2){ // 설명
			...
			}else if (e.GetMentsu(0,m0) && e.GetMentsu(1,m1) && e.GetAtamaMentsu(kk2,2,m2)) return true; // 설명
		}
		v[0].mmmm35=0; // 설명
		return ok;
	},
...
//		DBGPRINT((_T("GetMentsu col=%d mmmm=%X\r\n"),col,mmmm));
	...
	GetAtamaMentsu:function(nn,col,m){ // 설명
		var e=this;
		var a=(7<<(24-nn*3));
		var b=(2<<(24-nn*3));
	...
	GetMentsu9:function(mmmm,col,m,v){ // 설명
		// 설명
		var s=-1; // 설명
		var i, a=(m&7), b=0, c=0;
		for(i=0;i<7;++i){
			if (m==0x6DB) break; // 설명
			switch(a){
			case 4:mmmm<<=8, mmmm|=(col*7+i+1), b+=1, c+=1; // 설명
			case 3: // 설명
				if (((m>>3)&7)>=3+b && ((m>>6)&7)>=3+c) s=i, b+=3, c+=3;// 설명
	...
	GetMentsu9Fin:function(col,m){ // 설명
		var e=this;
		var v=e.v;
		var mm=[0,0,0];
...
//		document.write("GetMentsu9Fin col="+col+" n="+n+"<br>");
	...
	GetAtamaMentsu8Fin:function(nn,col,m){ // 설명
		var e=this;
		var v=e.v;
		var mmmm=v[0].mmmm35;
		var mm=[0,0,0];
