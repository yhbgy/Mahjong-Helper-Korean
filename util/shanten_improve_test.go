...
func TestCalculateShantenWithImproves14Closed(t *testing.T) {
	...
	tiles = "455678m 11566p 234s" // TODO: 설명 보완
	...
	tiles = "2389m 4566p 2289s 44z" // 설명
	tiles = "11233456789m 333p"     // 설명
	...
	playerInfo.LeftTiles34 = InitLeftTiles34WithTiles34(MustStrToTiles34("1366668888m 456p 55666677s")) // 설명
...
func TestCalculateMeld(t *testing.T) {
	...
	t.Log("원래 손패" + NumberToChineseShanten(result.Shanten))
	t.Log(result)
	...
	t.Log("울고 난 뒤" + NumberToChineseShanten(shanten))
	for _, result := range results {
		t.Log(result)
	}
	t.Log("울고 난 뒤" + NumberToChineseShanten(shanten+1))
	for _, result := range incShantenResults {
		t.Log(result)
	}
}
...
func TestBestDiscard(t *testing.T) {
	...
	// 텐파이
	assert.Equal("7m", bestHumanDiscardTile(t, "123667m 234p 345s 55z", ""))   // 설명
	assert.Equal("6m", bestHumanDiscardTile(t, "123667m 234p 345s 44z", ""))   // C3Q4 핑후
	assert.Equal("4m", bestHumanDiscardTile(t, "134m 123567p 12355s", ""))     // C3Q5 삼색동순
	assert.Equal("4m", bestHumanDiscardTile(t, "134m 123567p 12355s", "5p"))   // C3Q5 삼색동순
	assert.Equal("4s", bestHumanDiscardTile(t, "234456m 11567p 468s", ""))     // 설명
	assert.Equal("1m", bestHumanDiscardTile(t, "1234m 345789p 567s 3z", "3z")) // 설명
	assert.Equal("5s", bestHumanDiscardTile(t, "345m 345789p 3455s 4z", ""))   // 설명
	assert.Equal("7m", bestHumanDiscardTile(t, "234788m 234567s 33z", "8m"))   // 설명
	assert.Equal("8m", bestHumanDiscardTile(t, "234788m 234567s 33z", "3z"))   // 설명
	assert.Equal("7m", bestHumanDiscardTile(t, "334557m 222p 789s 33z", "9s")) // 설명
...
func TestQ300(t *testing.T) {
	...
	assert.Equal("8p", bestHumanDiscardTile2(t, "67778p 1122345s 77z", "2z"), "쓰모 시 산안커는 아직 고려하지 않음") // 설명
	...
	assert.Equal("2p", bestHumanDiscardTile2(t, "135m 11240667p 789s", "9s"), "마작은 쓰모 게임")            // 설명
	...
	assert.Equal("2s", bestHumanDiscardTile2(t, "55678m 3467p 24668s", "6p"), "6s도 가능")               // 설명
	...
	assert.Equal("3m", bestHumanDiscardTile2(t, "3356m 23478p 56777s", "7p"), "6m도 가능")               // 설명
	assert.Equal("1p", bestHumanDiscardTile2(t, "3456m 137899p 4578s", "1m"), "3m도 가능")               // 설명
	assert.Equal("8s", bestHumanDiscardTile2(t, "34056m 2224p 23468s", "8m"), "개량 영향 고려")             // 설명
}
