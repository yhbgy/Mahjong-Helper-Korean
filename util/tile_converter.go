...
func StrToTile34(humanTile string) (tile34 int, isRedFive bool, err error) {
	defer func() {
		if er := recover(); er != nil {
			err = fmt.Errorf("[StrToTile34] %#v 인자 오류: %s", er, humanTile)
		}
	}()

	wrongHumanTileError := errors.New("[StrToTile34] 인자 오류: " + humanTile)
...
func StrToTiles34(humanTiles string) (tiles34 []int, numRedFives []int, err error) {
	defer func() {
		if er := recover(); er != nil {
			err = errors.New("[StrToTiles34] 인자 오류: " + humanTiles)
		}
	}()
	...
	if humanTiles == "" {
		return nil, nil, errors.New("[StrToTiles34] 인자 오류: 처리할 손패가 비어 있으면 안 됩니다")
	}
	...
	for _, split := range strings.Split(humanTiles, " ") {
		...
		if len(split) < 2 {
			return nil, nil, errors.New("[StrToTiles34] 인자 오류: " + humanTiles)
		}
		...
		for _, c := range split[:len(split)-1] {
			...
			tiles34[tile34]++
			if tiles34[tile34] > 4 {
				return nil, nil, fmt.Errorf("[StrToTiles34] 인자 오류: %s 에 같은 패가 4장 넘게 있습니다", humanTiles)
			}
			if isRedFive {
